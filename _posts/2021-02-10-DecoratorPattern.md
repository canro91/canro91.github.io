...
I've been working with Stripe client to take payments. Depending on the volume of requests you make to the Stripe API, you might exceed the maximum number of requests per second. This could happen with any third-party API. This is how we can implement a retry mechanism using the Decorator pattern in C#.

**With the Decorator pattern, an object wraps another object to extend its responsabilities, without modifying its existing behavior.** Unlike an Adapter, a Decorator keeps the same public API of the object being decorated. Often, the Decorator pattern is use to add orthogonal responsibilities like logging, caching and retry logic to services.
...
We want to retry the method `CreateAsync` if it reaches the maximum number of allowed requests by Stripe in a given time.
...
```csharp
new RetryablePaymentIntentService(new PaymentIntentService());
```
...
Voilà! That's how we can implement the Decorator pattern to retry API calls. We can also use the decorator pattern to bring logging or caching to our services. Check how you can use the Decorator pattern to [add a cache layer]({% post_url 2020-06-29-HowToAddACacheLayer %}) to an ASP.NET Core using Redis.
